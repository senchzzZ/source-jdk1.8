...
public abstract class AbstractQueuedSynchronizer
     ...
     * CANCELLED.
     */
    //head节点。如果存在，waitStatus肯定不是CANCELLED状态
     ...
     */
    /**
     * 独占锁.尝试获取资源,成功返回true,具体实现交由自定义同步器实现
     */
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&
            acquireQueued(addWaiter(Node.EXCLUSIVE), arg))//获取许可失败，入列等待
            selfInterrupt();
    }
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        private void doSignal(Node first) {
            do {
                if ( (firstWaiter = first.nextWaiter) == null)//向后查找有效firstWaiter节点
                    lastWaiter = null;
        ...
        private void unlinkCancelledWaiters() {
            ...
            while (t != null) {
                ...
                if (t.waitStatus != Node.CONDITION) {
                    //节点等待状态不为CONDITION就解除链接
                    t.nextWaiter = null;
                    if (trail == null)
                        firstWaiter = next;//继续向后查找
                    else
